import classes from './arrow.module.css';
export default function HeroCardArrow({ direction, onClick }) {
  if (direction !== 'right' && direction !== 'left') {
    return <div>Wrong arrow direction</div>;
  }

  return (
    <div className={classes['hero-carousel-arrows']} onClick={onClick}>
      <div className={classes.arrow} role='button'>
        <img
          className={classes['arrow-' + direction]}
          src='data:image/svg+xml;base64,'
          alt={'Scroll' + direction}
        />
      </div>
    </div>
  );
}
