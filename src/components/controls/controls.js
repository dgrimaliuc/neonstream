import styles from './controls.module.css';

export default function Controls({ onLeftClick, onRightClick }) {
  return (
    <>
      <div className={styles.controls}>
        <span className={styles.control} onClick={onLeftClick}>
          <img
            className={styles['control-left']}
            src='data:image/svg+xml;base64,'
            alt='scroll left'
          />
        </span>
        <span className={styles.control} onClick={onRightClick}>
          <img
            className={styles['control-right']}
            src='data:image/svg+xml;base64,'
            alt='scroll right'
          />
        </span>
      </div>
    </>
  );
}
